"""
plot_feasible_by_task.py

Strip-scatter of normalized feasible improvement across the 3 DNA tasks.
Each dot = one feasible-improving step.  Color = method.
Normalization: (score - t0) / (task_best - t0)
  task_best = max raw score seen by ANY method on that task,
...
METHODS = ["GRACE", "AdaLead", "FastSeqProp", "Constrained BO", "Grad. Guided Evol."]

FNAME = {
    "GRACE":               "grace_lagrangian",
    "AdaLead":        "simulated_annealing",
    "FastSeqProp":       "unconstrained",
    "Constrained BO":      "botorch_baseline",
    "Grad. Guided Evol.":  "directed_evolution",
}
...
COLORS = {
    ...
    "AdaLead":        "#de6e56",
    "FastSeqProp":       "#969696",
    "Constrained BO":      "#41ab5d",
    "Grad. Guided Evol.":  "#7b3294",
}
...
def get_scores(path):
    ...
    # Use real_scores if present, otherwise scores — but be consistent per file
...
def collect_raw_points(task_dir, starts, fname_map, method):
    ...
    files = (
        [task_dir / (fname_map[method] + ".json")]
        if starts is None
        else [task_dir / s / (fname_map[method] + ".json") for s in starts]
    )
    points = []
    for p in files:
        if not p.exists():
        ...
        for i in range(use_n):
            ...
            for c in constraints:
                ...
                viol += (max(0.0, drift - eps) if ctype == "absolute"
...
def task_best_score(task_dir, starts, fname_map):
    """Max raw score any method achieves on this task (from same score source)."""
    best = None
    for m in METHODS:
...
def collect_normalized(task_dir, starts, fname_map, method):
    ...
    for score, t0 in points:
        denom = best - t0
        if denom > 1e-12:
            out.append(min(1.0, (score - t0) / denom))   # clip for safety
    return np.array(out)


# ── pre-compute all points ────────────────────────────────────────────────────

all_data = {}   # (ti, mi) -> np.array of normalized values
...
def draw_strip(ax, task_indices):
    ...
    for ti_local, ti_global in enumerate(task_indices):
        for mi, m in enumerate(METHODS):
            ...
            if len(vals) == 0:
                # "No improvement" marker: bold × at y=0
                ax.scatter([xc], [0.0],
                           marker='x', s=60,
                           color=COLORS[m], alpha=1.0,
                           linewidths=1.8, zorder=6)
                ax.text(xc, -0.015, "n/a",
            ...
            # boxplot (transparent fill, solid border)
            bp = ax.boxplot(
                vals,
                positions=[xc],
...
for ext in ("svg", "png"):
    out = OUT_DIR / f"feasible_by_task_dna.{ext}"
